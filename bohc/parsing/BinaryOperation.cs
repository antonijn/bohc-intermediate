...
namespace bohc.parsing
{
	public class BinaryOperation : Expression
	{
		...
		{
			return c.getFunctions(op, c).SingleOrDefault(x => x.parameters.First().type == other).returnType;
		}

		private static typesys.Type getOtherTypeArith(string op, typesys.Type other, typesys.Class c, bool shouldThrow)
		{
			return c.getFunctions(op, c).SingleOrDefault(x => x.parameters.Last().type == other).returnType;
		}

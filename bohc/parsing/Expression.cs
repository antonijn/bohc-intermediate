...
namespace bohc.parsing
{
	public abstract class Expression
	{
		...
		private static bool analyzeName(ref Expression last, List<typesys.Variable> vars, ref int i, ref string nxt, string str, ts.File file)
		{
			string next = nxt;
		...
		private static int solveIdentifierForType(ref Expression expr, List<typesys.Variable> vars, int i, string next, string str, ts.File file, typesys.Type type)
		{
			...
			{
				// function
				typesys.Function f = getCompatibleFunction(ref i, next, str, file, functions);
				FunctionCall call = new FunctionCall(f, expr);
				expr = call;
		...
		/// </summary>
		private static typesys.Function getCompatibleFunction(ref int i, string next, string str, ts.File file, IEnumerable<typesys.Function> functions)
		{
		...
		private static bool areParamsCompatible(IEnumerable<Expression> parameters, typesys.Function func)
		{
			using (IEnumerator<Expression> exprsns = parameters.GetEnumerator())
			{
				using (IEnumerator<typesys.Parameter> paramsf = func.parameters.GetEnumerator())
				{
					if (parameters.Count() > 0)
					{
						do
						{
							if (exprsns.Current.getType() != paramsf.Current.type)
							{
								return false;
							}
						}

#pragma once

enum p2p4eB_mypackEnumExample;
struct p2p4c9_mypackMainClass;
struct p3p3c6_bohstdObject;
struct p3p3c6_bohstdString;
struct p3p3c4_bohstdType;
struct p3p3c9_bohstdException;
struct p3p3cD_bohstdStringBuilder;
struct p3p3p7c7_bohstdinteropInterop;
struct p3p3c17_bohstdBox_my_pack_EnumExample;
struct p3p3c14_bohstdArray_boh_std_String;
struct p3p3cA_bohstdArray_char;
struct p3p3c9_bohstdArray_int;
struct p3p3c22_bohstdArray_boh_std_Array_boh_std_String;
struct p3p3i1A_bohstdICollection_boh_std_String;
struct p3p3i10_bohstdICollection_char;
struct p3p3iF_bohstdICollection_int;
struct p3p3i28_bohstdICollection_boh_std_Array_boh_std_String;
struct p3p3i21_bohstdIIndexedCollection_boh_std_String;
struct p3p3i17_bohstdIIndexedCollection_char;
struct p3p3i16_bohstdIIndexedCollection_int;
struct p3p3i2F_bohstdIIndexedCollection_boh_std_Array_boh_std_String;
struct p3p3i18_bohstdIIterator_boh_std_String;
struct p3p3iE_bohstdIIterator_char;
struct p3p3iD_bohstdIIterator_int;
struct p3p3i26_bohstdIIterator_boh_std_Array_boh_std_String;
struct p3p3c9_bohstdList_char;
struct p3p3c8_bohstdList_int;
#include "p3p3p7c8_bohstdinteropPtr_char.h"

struct p3p3p7c7_bohstdinteropVoidPtr
{
#if defined(PF_DESKTOP64)
	int64_t f_ptr;
#endif
#if defined(PF_DESKTOP32)
	int32_t f_ptr;
#endif
};

#include "boh_internal.h"
#include "function_types.h"
#include <stdint.h>
#include <stddef.h>
#include <uchar.h>
#include <setjmp.h>
#include "p2p4eB_mypackEnumExample.h"
#include "p2p4c9_mypackMainClass.h"
#include "p3p3c6_bohstdObject.h"
#include "p3p3c6_bohstdString.h"
#include "p3p3c4_bohstdType.h"
#include "p3p3c9_bohstdException.h"
#include "p3p3cD_bohstdStringBuilder.h"
#include "p3p3p7c7_bohstdinteropInterop.h"
#include "p3p3c17_bohstdBox_my_pack_EnumExample.h"
#include "p3p3c14_bohstdArray_boh_std_String.h"
#include "p3p3cA_bohstdArray_char.h"
#include "p3p3c9_bohstdArray_int.h"
#include "p3p3c22_bohstdArray_boh_std_Array_boh_std_String.h"
#include "p3p3i1A_bohstdICollection_boh_std_String.h"
#include "p3p3i10_bohstdICollection_char.h"
#include "p3p3iF_bohstdICollection_int.h"
#include "p3p3i28_bohstdICollection_boh_std_Array_boh_std_String.h"
#include "p3p3i21_bohstdIIndexedCollection_boh_std_String.h"
#include "p3p3i17_bohstdIIndexedCollection_char.h"
#include "p3p3i16_bohstdIIndexedCollection_int.h"
#include "p3p3i2F_bohstdIIndexedCollection_boh_std_Array_boh_std_String.h"
#include "p3p3i18_bohstdIIterator_boh_std_String.h"
#include "p3p3iE_bohstdIIterator_char.h"
#include "p3p3iD_bohstdIIterator_int.h"
#include "p3p3i26_bohstdIIterator_boh_std_Array_boh_std_String.h"
#include "p3p3c9_bohstdList_char.h"
#include "p3p3c8_bohstdList_int.h"

extern struct p3p3c4_bohstdType * typeof_p3p3p7c7_bohstdinteropVoidPtr(void);

extern struct p3p3p7c7_bohstdinteropVoidPtr new_p3p3p7c7_bohstdinteropVoidPtr_112000b3(int64_t p_ptr);
extern struct p3p3p7c7_bohstdinteropVoidPtr new_p3p3p7c7_bohstdinteropVoidPtr_adeaa357(int32_t p_ptr);
extern struct p3p3p7c7_bohstdinteropVoidPtr new_p3p3p7c7_bohstdinteropVoidPtr_35cf4c(void);

#if defined(PF_DESKTOP64)
extern void p3p3p7c7_bohstdinteropVoidPtr_m_this_112000b3(struct p3p3p7c7_bohstdinteropVoidPtr * const self, int64_t p_ptr);
#endif
#if defined(PF_DESKTOP32)
extern void p3p3p7c7_bohstdinteropVoidPtr_m_this_adeaa357(struct p3p3p7c7_bohstdinteropVoidPtr * const self, int32_t p_ptr);
#endif
#if defined(PF_DESKTOP64)
extern int64_t p3p3p7c7_bohstdinteropVoidPtr_m_toInt_35cf4c(struct p3p3p7c7_bohstdinteropVoidPtr * const self);
#endif
#if defined(PF_DESKTOP32)
extern int32_t p3p3p7c7_bohstdinteropVoidPtr_m_toInt_35cf4c(struct p3p3p7c7_bohstdinteropVoidPtr * const self);
#endif
extern void p3p3p7c7_bohstdinteropVoidPtr_m_this_35cf4c(struct p3p3p7c7_bohstdinteropVoidPtr * const self);
extern void p3p3p7c7_bohstdinteropVoidPtr_m_static_0(void);


